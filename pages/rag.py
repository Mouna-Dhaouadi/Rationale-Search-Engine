...
similarity_threshold = cols_llm.text_input("Enter the cosine similarity threshold:", value=0.6)

if user_input:
    relevant_changes_df = get_relevant_changes(user_input, triples_file_path, similarity_threshold)
    cols_llm.write('%s similar commits' % len(relevant_changes_df))
    cols_llm.write( relevant_changes_df)
    ...
    prompt = construct_propmt(module, user_input, list(relevant_changes_df['Url'])[:int(context_size)])
...
contradiction_threshold = cols_llm_2.text_input("Enter the contradiction threshold:", value=0.6)

if user_input:
    with st.spinner("Wait for it...", show_time=True):
        contradictory_changes_df = get_contradictory_changes(user_input, triples_file_path, contradiction_threshold)
        
    cols_llm_2.write('%s contradictory commits' % len(contradictory_changes_df))
    cols_llm_2.write( contradictory_changes_df)

    cols_llm_2.subheader('Get ChatGPT to double check these potential issues')
    ...
    prompt = construct_propmt_2(module, user_input, list(contradictory_changes_df['Url'])[:int(context_size)])
    response = ask_llm(user_input, prompt)
